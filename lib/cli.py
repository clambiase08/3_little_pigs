...
engine = create_engine("sqlite:///lib/db/3_little_pigs.db")
...
def display_main_menu():
    print_slowly(
        "Way back in Once upon a time time, there were three little pigs, all siblings. The first little big, Albert, was VERY lazy. He didn't want to work very hard so he wants to build his house out of straw."
    )
    print_slowly(
        "The second little pig, Brenda, worked a little bit harder but was still a little lazy so she wants to build her house out of twigs."
    )
    print_slowly(
        "The third little pig, Charlie, worked very hard and so he wants to build his house out of bricks."
    )
    print_kinda_slow(
        """
                                                          (
...
def display_current_scene():
    ...
    if choice == "C":
        ...
        CURRENT_SCENE = (
            session.query(Scene)
            .filter(Scene.scene_num == CURRENT_SCENE.choice_A_next_scene)
            .first()
        )
        ...
        CURRENT_SCENE = (
            session.query(Scene)
            .filter(Scene.scene_num == CURRENT_SCENE.choice_B_next_scene)
            .first()
        )
        Storyline(story_id=CURRENT_STORY.id, scene_id=CURRENT_SCENE.id)
        display_current_scene()
